...
def strip_prefix_if_present(state_dict, prefix):
    """
    Strip the given prefix from the keys of the state_dict if it exists.
    ...
    Returns:
        dict: A new state_dict with the prefix stripped from the keys.
    """
    if not any(key.startswith(prefix) for key in state_dict.keys()):
        return state_dict  # Return the original state_dict if no prefix is found
