...
from perceptual1 import LossNetwork
...
def main():
    ...
    g1_dataset = TrainingDataset(3, crop_size, train_data_dir, labeled_name)  # ảnh nhiễu loại g1
    g1_loader_style = data.DataLoader(g1_dataset, batch_size=args.batch_size_style, shuffle=True, num_workers=args.num_workers,
                                        pin_memory=True, drop_last=True)
    g1_loader_style_iter = enumerate(g1_loader_style)

    g4_dataset = TrainingDataset(4, crop_size, train_data_dir, labeled_name)  # ảnh nhiễu loại g1
    g4_loader_style = data.DataLoader(g4_dataset, batch_size=args.batch_size_style, shuffle=True, num_workers=args.num_workers,
                                        pin_memory=True, drop_last=True)
    g4_loader_style_iter = enumerate(g4_loader_style)
    ...
    for i_iter in tqdm(range(0, args.num_steps)): 
        ...
        try:    #g1
            _, batch = g1_loader_style_iter.__next__()
        except StopIteration:
            g1_loader_style = data.DataLoader(g1_dataset, batch_size=args.batch_size_style, shuffle=True, num_workers=args.num_workers,
                                    pin_memory=True, drop_last=True)    #load raindrop dataset (drop last)
            g1_loader_style_iter = enumerate(g1_loader_style) 
            _, batch = g1_loader_style_iter.__next__()
        g1_img, gt_g1_img = batch

        try:    #g4
            _, batch = g4_loader_style_iter.__next__()
        except StopIteration:
            g4_loader_style = data.DataLoader(g4_dataset, batch_size=args.batch_size_style, shuffle=True, num_workers=args.num_workers,
                                    pin_memory=True, drop_last=True)    #load raindrop dataset (drop last)
            g4_loader_style_iter = enumerate(g4_loader_style) 
            _, batch = g4_loader_style_iter.__next__()
        g4_img, gt_g4_img = batch
        ...
        g1_vec = StyleFilter(g1_img)
        g4_vec = StyleFilter(g4_img)

        # gt_vec = StyleFilter(torch.cat([gt_raindrop_img, gt_rain_img, gt_snow_img, gt_g1_img, gt_g4_img], dim=0))  #3*batch_size_style
        gt_vec = StyleFilter(torch.cat([gt_g1_img, gt_g4_img], dim=0))  #3*batch_size_style
        ...
        # Đảm bảo chỉ truy cập các chỉ số từ 0 đến 7 (vì batch size là 8)
        style_embeddings = torch.cat((
            torch.unsqueeze(g1_vec[0], 0), torch.unsqueeze(g4_vec[0], 0),
            torch.unsqueeze(g1_vec[1], 0), torch.unsqueeze(g4_vec[1], 0),
            torch.unsqueeze(g1_vec[2], 0), torch.unsqueeze(g4_vec[2], 0),
            torch.unsqueeze(g1_vec[3], 0), torch.unsqueeze(g4_vec[3], 0),
            # Các phần tử của gt_vec phải nằm trong phạm vi chỉ số hợp lệ (0-7)
            torch.unsqueeze(gt_vec[0], 0), torch.unsqueeze(gt_vec[1], 0), torch.unsqueeze(gt_vec[2], 0), torch.unsqueeze(gt_vec[3], 0),
            # Các chỉ số phải nhỏ hơn 8
            torch.unsqueeze(gt_vec[4], 0), torch.unsqueeze(gt_vec[5], 0), torch.unsqueeze(gt_vec[6], 0), torch.unsqueeze(gt_vec[7], 0)
        ), 0)
        ...
        style_labels = torch.LongTensor([3, 4, 3, 4, 3, 4, 3, 4, 
                                         5, 5, 5, 5, 5, 5, 5, 5])  # style labels of style embeddings
