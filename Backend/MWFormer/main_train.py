...
def main():
    ...
    g1_dataset = TrainingDataset(3, crop_size, train_data_dir,labeled_name)
    g1_loader = data.DataLoader(g1_dataset,batch_size=11, shuffle=True, num_workers=args.num_workers,
                                        pin_memory=True, drop_last=True)    #load g1 dataset
    g1_loader_iter = enumerate(g1_loader)

    g2_dataset = TrainingDataset(4, crop_size, train_data_dir,labeled_name)
    g2_loader = data.DataLoader(g2_dataset,batch_size=11, shuffle=True, num_workers=args.num_workers,
                                        pin_memory=True, drop_last=True)    #load g2 dataset
    g2_loader_iter = enumerate(g2_loader)



    full_dataset = TrainingDataset(5, crop_size, train_data_dir,labeled_name)
    ...
    for i_iter in tqdm(range(start_iter, args.num_steps)): 
        ...
        try:    #g1
                _, batch_g1 = g1_loader_iter.__next__()
        except StopIteration:
            g1_loader = data.DataLoader(g1_dataset, batch_size=4, shuffle=True, num_workers=args.num_workers,
                            pin_memory=True, drop_last=True)    #load raindrop dataset (drop last)
            g1_loader_iter = enumerate(g1_loader) 
            _, batch_g1 = g1_loader_iter.__next__()
        g1_img, gt_g1_img = batch_g1
        imgs_g1 = Variable(g1_img).to(device)

        try:    #g2
                _, batch_g2 = g2_loader_iter.__next__()
        except StopIteration:
            g2_loader = data.DataLoader(g2_dataset, batch_size=4, shuffle=True, num_workers=args.num_workers,
                            pin_memory=True, drop_last=True)    #load raindrop dataset (drop last)
            g2_loader_iter = enumerate(g2_loader) 
            _, batch_g2 = g2_loader_iter.__next__()
        g2_img, gt_g2_img = batch_g2
        imgs_g2 = Variable(g2_img).to(device)

        # imgs_input = torch.cat([imgs_raindrop, imgs_rain, imgs_snow], dim=0)
        # gt = torch.cat([gt_raindrop_img, gt_rain_img, gt_snow_img], dim=0)
        imgs_input = torch.cat([imgs_g1, imgs_g2], dim=0)
        gt = torch.cat([gt_g1_img, gt_g2_img], dim=0)
